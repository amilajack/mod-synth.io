...
class Q extends Slider {
  constructor(component_session_uid) {
    ...
    this.percentage = MathUtils.map(
      Session.SETTINGS[this.component_session_uid].settings.q,
      this.range.min,
      this.range.max,
      0,
      100,
  ...
  onSettingsChange(event) {
    if (event.component === this.component_session_uid) {
      let v = Session.SETTINGS[this.component_session_uid].settings.q;
      if (v >= 1000) {
        v =
          (Session.SETTINGS[this.component_session_uid].settings.q / 1000
          ).toFixed(1) + "k";
      }
      this.value.text = v;
    }
  ...
  onUpdate() {
    Session.SETTINGS[this.component_session_uid].settings.q = MathUtils.map(
      this.percentage,
      0,
      100,
      this.range.min,
