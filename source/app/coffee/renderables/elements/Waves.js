...
class Waves extends Slider {
  constructor(component_session_uid) {
    ...
    this.possibleValues = [
      ...
      AppData.WAVE_TYPE.TRIANGLE,
      AppData.WAVE_TYPE.SQUARE,
      AppData.WAVE_TYPE.SAWTOOTH
    ];
  ...
  onEnd(e) {
    ...
    if (this.lastValue === this.percentage) {
      let next =
        Session.SETTINGS[this.component_session_uid].settings.wave_type + 1;
  ...
  onUpdate() {
    ...
    ].settings.wave_type = MathUtils.map(
      this.percentage,
      0,
      100,
      0,
