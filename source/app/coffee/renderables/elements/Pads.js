...
class Pads extends PIXI.Container {
    ...
    removePads() {
        for (let i = 0, end = this.children.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
    ...
    addPads() {
        for (let i = 0, end = this.total, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
    ...
    onSettingsChange(event) {
        if (event.component === this.component_session_uid) {
            for (let i = 0, end = Session.SETTINGS[this.component_session_uid].settings.pattern.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
