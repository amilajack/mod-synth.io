...
class Filters extends Slider {
  constructor(component_session_uid) {
    ...
    this.possibleValues = [
      ...
      AppData.FILTER_TYPE.HIGHPASS,
      AppData.FILTER_TYPE.BANDPASS,
      AppData.FILTER_TYPE.LOWSHELF,
      AppData.FILTER_TYPE.HIGHSHELF,
      AppData.FILTER_TYPE.PEAKING,
      AppData.FILTER_TYPE.NOTCH,
      AppData.FILTER_TYPE.ALLPASS
    ];
  ...
  onEnd(e) {
    ...
    if (this.lastValue === this.percentage) {
      let next =
        Session.SETTINGS[this.component_session_uid].settings.filter_type + 1;
  ...
  onUpdate() {
    ...
    ].settings.filter_type = MathUtils.map(
      this.percentage,
      0,
      100,
      0,
