...
class PresetsPannel extends Pannel {
    ...
    build() {
        ...
        this.button_NEW = new SubmenuButton('Create new preset', AppData.ASSETS.sprite.textures['ic-add-32.png']);
        ...
        this.description = new PIXI.Text('You can\'t add presets to the default patch.', AppData.TEXTFORMAT.MENU_DESCRIPTION);
        ...
        this.saved = new PIXI.Text('AVAILABLE PRESETS', AppData.TEXTFORMAT.MENU_SUBTITLE);
        ...
        for (let i = 0, end = order.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
            ...
            const bt = new SubmenuButtonPreset(order[i].name, new Date(parseInt(order[i].date)).toLocaleDateString(), allowDelete);
    ...
    createPreset() {
        App.PROMPT.dispatch({
            ...
            onConfirm: data => {
                ...
                return Services.api.presets.save(Session.patch.uid, preset_id, data, snapshot => {
    ...
    attachButtonClick(bt, uid) {
        bt.buttonClick = () => {
            if (uid === Session.patch.preset) { return; }
    ...
    onPresetChanged() {
        ...
        for (let i = 0, end = this.elements.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
            this.elements[i].setCurrent(Session.patch.presets[Session.patch.preset].name.toUpperCase() === this.elements[i].label.text);
        }
        return null;
    }
}
