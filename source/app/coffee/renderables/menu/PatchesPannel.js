...
class PatchesPannel extends Pannel {
  ...
  build(data) {
    ...
    this.description = new PIXI.Text(
      "You need to login in order to save or load patches.",
    ...
    let bt = new SubmenuButtonPatch(
      Session.default.uid,
      new Date(parseInt(Session.default.date)).toLocaleDateString(),
      false
    );
  ...
  onPatchLoaded() {
    for (
      ...
      if (!Session.patch.name) {
        return;
      }
      this.elements[i].setCurrent(
        Session.patch.name.toUpperCase() === this.elements[i].label.text
      );
    }
    return null;
  }
