...
class LoginPannel extends Pannel {
  ...
  build() {
    ...
    if (Services.REFERENCE.auth().currentUser) {
      ...
      this.title = new PIXI.Text(
        `LOGGED VIA ${data.providerId.toUpperCase()}`,
      ...
      const twitter = new SubmenuButton(
        "twitter",
        AppData.ASSETS.sprite.textures["ic-twitter.png"]
      );
      twitter.buttonClick = this.handleTwitter;
      this.addChild(twitter);
      this.elements.push(twitter);

      const facebook = new SubmenuButton(
        "facebook",
        AppData.ASSETS.sprite.textures["ic-facebook.png"]
      );
      facebook.buttonClick = this.handleFacebook;
      this.addChild(facebook);
      this.elements.push(facebook);

      const github = new SubmenuButton(
        "github",
        AppData.ASSETS.sprite.textures["ic-github.png"]
      );
  ...
  align() {
    ...
    for (
      ...
      if (i === 0) {
        this.elements[i].y = data
          ? this.title.y + this.title.height + AppData.PADDING / 2
          : AppData.MENU_PANNEL;
      } else {
        this.elements[i].y =
