...
class MidiPannel extends Pannel {
  constructor(label) {
    ...
    this.description = new PIXI.Text(
      "You need to connect your MIDI enabled device in order to control the synthesizer",
    ...
    this.description.position.y =
      this.title.y + this.title.height + AppData.PADDING;
  ...
  onMidiStateChange(e) {
    ...
    for (let controller in this.controllers) {
      const bt = new SubmenuButtonMidi(
        controller,
        AppData.ASSETS.sprite.textures["ic-selection-inactive.png"]
      );
      bt.active = false;
      this.addChild(bt);
