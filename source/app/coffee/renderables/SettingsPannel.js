...
class SettingsPannel extends PIXI.Sprite {
    ...
    onToggle(value) {
        if (value.type) {
            ...
            switch (Session.GET(value.component_session_uid).type_uid) {
                case AppData.COMPONENTS.NSG: s = new NsgSettings(value.component_session_uid); break;
                case AppData.COMPONENTS.OSC: s = new OscSettings(value.component_session_uid); break;
                case AppData.COMPONENTS.ENV: s = new EnvSettings(value.component_session_uid); break;
                case AppData.COMPONENTS.FLT: s = new FltSettings(value.component_session_uid); break;
                case AppData.COMPONENTS.PTG: s = new PtgSettings(value.component_session_uid); break;
                case AppData.COMPONENTS.LFO: s = new LfoSettings(value.component_session_uid); break;
                default:
                    return;
            }
    ...
    removeAllFromSettings() {
        for (let i = 0, end = this.settingsHolder.children.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
    ...
    close() {
        this.pannelShow = false;
        TweenMax.to(this.theMask, 0.1, { y: AppData.SETTINGS_PANNEL_HEIGHT, onComplete: () => {
