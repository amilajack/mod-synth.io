...
class Picker extends PIXI.Container {
  ...
  onPickerShow(e) {
    ...
    if (e.elements) {
      ...
      for (
        let i = 0, end = e.elements.length, asc = 0 <= end;
        asc ? i < end : i > end;
        asc ? i++ : i--
      ) {
        const pos = this.getCenter(
          Math.floor(this.initialAngle + angleStep * i),
          Math.ceil(this.initialAngle + angleStep * (i + 1)),
          this.innerCircle,
          this.outterCIrcle
        );
  ...
  onPickerValue(e) {
    ...
    for (
      let i = 0, end = this.background.length, asc = 0 <= end;
      asc ? i < end : i > end;
      asc ? i++ : i--
    ) {
      if (i === 0) {
        this.context.moveTo(
          this.outterCIrcle + this.background[i].x,
          this.outterCIrcle + this.background[i].y
        );
      } else if (i === this.background.length - 1) {
        this.context.lineTo(
          this.outterCIrcle + this.background[0].x,
          this.outterCIrcle + this.background[0].y
        );
      } else {
        this.context.lineTo(
          this.outterCIrcle + this.background[i].x,
  ...
  removeElements() {
    for (
      let i = 0, end = this.elements.children.length, asc = 0 <= end;
      asc ? i < end : i > end;
      asc ? i++ : i--
    ) {
  ...
  draw() {
    ...
    if (!this.snap) {
      ...
      this.border = this.getSlide(
        this.initialAngle,
        curAngle,
        this.innerCircle,
        this.innerCircle + this.borderRadius
      );
    } else {
      const step = MathUtils.map(this.value, 0, 100, 0, this.steps - 1, true);
      const angleStep = (this.finalAngle - this.initialAngle) / this.steps;
      this.slide = this.getSlide(
        Math.floor(this.initialAngle + angleStep * step),
        Math.ceil(this.initialAngle + angleStep * (step + 1)),
        this.innerCircle,
        this.outterCIrcle
      );
      this.border = this.getSlide(
    ...
    for (
      i = 0, end = this.slide.length, asc = 0 <= end;
      asc ? i < end : i > end;
      asc ? i++ : i--
    ) {
      if (i === 0) {
        this.context.moveTo(
          this.outterCIrcle + this.slide[i].x,
          this.outterCIrcle + this.slide[i].y
        );
      } else if (i === this.slide.length - 1) {
        this.context.lineTo(
          this.outterCIrcle + this.slide[0].x,
          this.outterCIrcle + this.slide[0].y
        );
      } else {
        this.context.lineTo(
          this.outterCIrcle + this.slide[i].x,
    ...
    for (
      i = 0, end1 = this.border.length, asc1 = 0 <= end1;
      asc1 ? i < end1 : i > end1;
      asc1 ? i++ : i--
    ) {
      if (i === 0) {
        this.context.moveTo(
          this.outterCIrcle + this.border[i].x,
          this.outterCIrcle + this.border[i].y
        );
      } else if (i === this.border.length - 1) {
        this.context.lineTo(
          this.outterCIrcle + this.border[0].x,
          this.outterCIrcle + this.border[0].y
        );
      } else {
        this.context.lineTo(
          this.outterCIrcle + this.border[i].x,
