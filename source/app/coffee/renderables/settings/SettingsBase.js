...
class SettingsBase extends PIXI.Container {
  ...
  constructor(component_session_uid) {
    ...
    this.graphics.beginFill(
      AppData.COLORS[Session.SETTINGS[this.component_session_uid].type_uid]
    );
    ...
    this.label = new PIXI.Text(
      AppData.TITLE[Session.SETTINGS[this.component_session_uid].type_uid],
      AppData.TEXTFORMAT.SETTINGS_TITLE
    );
      ...
      () => {
        ...
        return (() => {
          ...
          for (let id in Session.patch.presets) {
            result.push(
              Services.api.presets.updateRemove(id, this.component_session_uid)
            );
          }
          return result;
        })();
  ...
  adjustPosition() {
    for (
      ...
      if (i === 0) {
        ...
        this.elements[i].x =
          this.elements[i - 1].x + this.elements[i - 1].width;
      }
      this.elements[i].y =
        Math.floor(AppData.SETTINGS_PANNEL_HEIGHT - AppData.ICON_SIZE_1) / 2;
    }
    return null;
  }
}
