...
class Tour {
  constructor() {
    ...
    for (
      let i = 0, end = this.steps.length, asc = 0 <= end;
      asc ? i < end : i > end;
      asc ? i++ : i--
    ) {
  ...
  handleStep() {
    ...
    this.moveTo(
      this.steps[this.stepIndex].align,
      this.steps[this.stepIndex].x,
      this.steps[this.stepIndex].y
    );
    if (this.stepIndex < this.steps.length - 1) {
      this.showIndicator();
      if (this.stepIndex === 0) {
        TweenLite.to(this.skip, 0.5, {
          autoAlpha: 1,
          bottom: "25%",
    ...
    switch (align) {
      case "tr":
        TweenLite.to(this.inner, 0.0, {
          css: { top: y, left: "initial", bottom: "initial", right: x }
        });
        break;
      case "tl":
        TweenLite.to(this.inner, 0.0, {
          css: { top: y, left: x, bottom: "initial", right: "initial" }
        });
        break;
      case "bl":
        TweenLite.to(this.inner, 0.0, {
          css: { top: "initial", left: x, bottom: y, right: "initial" }
        });
        break;
    }
    return null;
