...
class PhysicsEngine {
  ...
  down(mouseX, mouseY) {
    const body = this.getBodyAtPosition(
  ...
  createCircle(radius, x, y, type) {
    const polygonShape = new Box2D.Collision.Shapes.b2CircleShape(
      radius / this.worldScale
    );
  ...
  removeBounds() {
    for (
      let i = 0, end = this.bounds.length, asc = 0 <= end;
      asc ? i < end : i > end;
      asc ? i++ : i--
    ) {
