    ...
    this.api = {
      ...
      patches: {
        getAll(callback) {
          if (Services.REFERENCE.auth().currentUser) {
            ...
            Services.PATCHES
              .orderByChild("author")
              .equalTo(
                Services.REFERENCE.auth().currentUser.providerData[0].uid
              )
              .once("value", callback);
          }
          return null;
        ...
        save(patch_name, callback) {
          if (Services.REFERENCE.auth().currentUser) {
            ...
            const providerData = Services.REFERENCE.auth().currentUser
              .providerData[0];
      ...
      presets: {
        ...
        remove(patch_id, callback) {
          ...
          preset.remove(error => {
            if (!error) {
              return Services.api.presets.loadAll(
                Session.patch.uid,
                snapshot => {
                  Session.patch.preset = "default";
        ...
        updateRemove(id, component_session_uid) {
          ...
          component.remove(error => {
            if (!error) {
              delete Session.patch.presets[id].components[
                component_session_uid
              ];
            }
            return null;
          });
