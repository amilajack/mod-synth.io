...
class Instrument {
    ...
    getNextAvailableChannelStrip() {
        ...
        if (this.channels.length === 0) {
            ...
            for (let i = 0, end = this.channels.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
    ...
    onNoteOn(data) {
        ...
        for (let i = 0, end = this.components.length, asc = 0 <= end; asc ? i < end : i > end; asc ? i++ : i--) {
            ...
            switch (component.type_uid) {
                case AppData.COMPONENTS.OSC: case AppData.COMPONENTS.NSG:
    ...
    add(data) {
        ...
        switch (data.type_uid) {
            case AppData.COMPONENTS.NSG: component = new NoiseGenerator(data); break;
            case AppData.COMPONENTS.OSC: component = new Oscillator(data); break;
            case AppData.COMPONENTS.ENV: component = new Envelope(data); break;
            case AppData.COMPONENTS.FLT: component = new Flt(data); break;
            case AppData.COMPONENTS.PTG: component = new PatternGate(data); break;
            case AppData.COMPONENTS.LFO: component = new Lfo(data); break;
        }
